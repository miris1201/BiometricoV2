...
class cFunction
{
    ...
    function fn_paginacion($pagina, $numeroTotalPaginas, $raiz, $ruta_paginado, $busqueda)
    {
        ...
        if ($pagina > 1 && $numeroTotalPaginas > 1) { // Mostrar si no es la primera pagina
            $paginado .= "<a href='" . $raiz . $ruta_paginado . "&pag=1$busqueda'  title='Primera página' class='btn ink-reaction btn-floating-action btn-sm' style='float:left; background-color: #31B189;'>
        ...
        if ($pagina > 1) {
            $paginado .= "<a href='" . $raiz . $ruta_paginado . "&pag=" . ($pagina - 1) . "$busqueda' 
                            ...
                            style='float:left; background-color: #31B189;'>
    ...
    function formatearFechaActas($fecha)
    {   // formatear fecha y hora
        if (strpos($fecha, '/') !== false) {
            $fecha = str_replace('/', '-', $fecha);
            $fecha = date('Y-m-d H:i', strtotime($fecha));
